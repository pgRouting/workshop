...
pgRouting functions provide a "loexercise-level" interface to algorithms and return ordered identifiers rather than routes with geometries.
Creating a complex queries, views or wrapper functions, can be used to connect to a high level application.

.. note::

    pgRouting only supports loexercise-level functions and it is the user responiblility to write their own wrapper functions for their own use cases.
...
.. note::
    * For this chapter, all the examples will return a human readable geometry for analysis, except :ref:`Exercise 12 <exercise-12>`.
